...
bool_t objs_delete_on_mplhead(mplhead_t* mplhdp,adr_t fradr)
{
    ...
    //将释放对象的地址赋值给mh_firtfreadr域
    mplhdp->mh_firtfreadr=fradr;
    //将以前的mh_irtfreadr域的值赋给释放对象的下一个对象指针域
    *nextp=retadr;
...
adr_t objs_new_on_mplhead(mplhead_t* mplhdp)
{
    ...
    //把当前空闲对象的指针赋给retadr
    retadr=mplhdp->mh_firtfreadr;
    //(retadr+adrsz)就是当前对象后的下一个对象的指针
    nextp=(adr_t*)(retadr+adrsz);
    //让mh_firtfreadr指向下一个空闲对象
    mplhdp->mh_firtfreadr=*nextp;
    //把当前对象的下一个对象指针域设为NULL
    *nextp=NULL;
    //增加对象分配计数
    mplhdp->mh_afindx++;
    //返回分配内存空间的地址
...
adr_t page_new_on_mplhead(mplhead_t* mplhdp)
{
    ...
    {
        //把当前空闲的pglmap_t结构保存到map中
        map=mplhdp->mh_firtpmap;
        //指向下一个空闲的pglmap_t结构
        mplhdp->mh_firtpmap=map->pgl_next;
        //将当前空闲的pglmap_t结构断掉链接
        map->pgl_next=NULL;
        //增加分配计数
        mplhdp->mh_afindx++;
        //返回当前空闲pglmap_t结构中pql_start域的值
...
mplhead_t* objs_mpool_init(kmempool_t* kmplockp,mplhead_t* initp,size_t msize,adr_t start,adr_t end)
{
    ...
    for(;;)
    {
        ...
        //把下一个对象指针的值放入这个地址的内存单元中
        *nexp=(sadr+npsz);
        //运行完这行代码，sadr中的值就是下一个对象的开始地址
        sadr+=npsz;
    ...
    sadr-=npsz;
    nexp=(adr_t*)(sadr+adrsz);
    //把最后一个对象后的下一个对象指针设为NULL，表示其后没无对象
    *nexp=NULL;
