...
{
    uint_t intoset=hal_retn_intnr(); //获取主中断源
  
    switch(intoset) //依次判断和处理，子中断源和外部中断源
    {
    
        case EINT4_7:
            hal_eint_distr(sframe,intoset,EI4_7_PNDBTS,EI4_7_PNDBTE); //处理外部中断
            break;
        case EINT8_23:
            hal_eint_distr(sframe,intoset,EI8_23_PNDBTS,EI8_23_PNDBTE);
            break;    
        case INT_CAM:
            hal_sint_distr(sframe,intoset,ICAM_PNDBTS,ICAM_PNDBTE);//处理子中断
            break;
        case INT_WDT_AC97:
    ...
    hal_clear_intpnd(intoset); //清除主中断源挂起寄存器中的相关位

    krlsched_chkneed_pmptsched(); //检查是否需要进程调度
    return;
}
