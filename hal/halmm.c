...
void mmapdsc_t_init(mmapdsc_t* mmp,adr_t phyadr,adr_t phyadre,u32_t allcount,u32_t flgs)
{
    ...
    mmp->map_phyadr=phyadr; //内存块开始地址
    mmp->map_phyadrend=phyadre; // 内存块的结束地址
    mmp->map_allcount=allcount;// 分配计数
    mmp->map_flg=flgs; //相关的标志
    return;
}
...
bool_t hal_onmapdsc_freeblks(adr_t frebadr,u32_t mflg, u32_t mask,alcfrelst_t* aflp,alcfrelst_t* mvtoaflp)
{
    ...
    //检查一下计算出的map_allcount中的对应位是否位0，为0就表示出错了
    if(((map->map_allcount>>bitnr)&1)!=1)
    {
       return FALSE;
    }
    
    //把map_allcount的对应位清0，表示已经释放了
    map->map_allcount&=(~(1<<bitnr));
    //如果map_allcount中表示内存空间块状态相应的位都是0，就移动到最后一个alcfrelst_t结构中
